...
export function InvitationDetails({ invitation, onBack, onEdit }: InvitationDetailsProps) {
  ...
  // Sending is gated on `send-invitation`, guest CRUD on `edit-invitation` —
  // the same split the API enforces on these routes.
  const maySend = can('send-invitation');
  const mayEditGuests = can('edit-invitation');
  /** Bodyless send — fans out to every not-yet-messaged guest, no credit spent. */
  const canSendToUnsent = maySend && canBeSent && !isBarcodeSuspended && hasUnsentGuests;
  /** Replacement send — needs credit and the picker. */
  const canSendReplacement =
    maySend && canBeSent && !isBarcodeSuspended && isSent && supportsReplacementSend && resendCredit > 0;
