...
export function EventEditForm({ event, onBack, onSave }: EventEditFormProps) {
  ...
  const [welcomeMessage, setWelcomeMessage] = useState(event?.welcomeMessage || 'Welcome to our event!');
    ...
    >
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-[2rem] border border-secondary/5 shadow-sm w-full max-w-2xl mx-auto crystal-accent">
         <h2 className={cn("text-2xl font-medium text-secondary mb-8", dir === 'ltr' ? 'font-serif' : 'font-arabic font-bold')}>
            {event ? (t('editEvent' as any) || 'Edit Event') : (t('addEvent' as any) || 'Add Event')}
         </h2>
         <form onSubmit={handleSubmit} className="space-y-5">
           ...
           <div>
             <label className="block text-sm font-medium text-secondary/80 mb-1.5 ml-1 flex items-center gap-2">
               ...
               Hall Location (Google Maps Link) <span className="text-secondary/50 text-xs font-normal ml-1">(Optional)</span>
             </label>
             <input
               type="url"
               value={hallLocation}
           ...
           <div>
             <label className="block text-sm font-medium text-secondary/80 mb-1.5 ml-1 flex items-center gap-2">
               ...
               WhatsApp Welcome Message <span className="text-red-500">*</span>
             </label>
           ...
           <div className="pt-2">
             <label className="flex items-center gap-2 cursor-pointer">
               ...
               />
               <span className="text-sm text-secondary/80 font-medium">Is Paid?</span>
             </label>
           </div>
             ...
             <div className="pt-2">
               ...
               <div className="flex items-center gap-6">
                 <label className="flex items-center gap-2 cursor-pointer">
                   ...
                   />
                   <span className="text-sm text-secondary/80 font-medium">One Payment</span>
                 </label>
                 <label className="flex items-center gap-2 cursor-pointer">
                   <input 
                     type="radio" 
                     ...
                     className="w-4 h-4 text-primary bg-white border-secondary/30 focus:ring-primary/30"
                   />
                   <span className="text-sm text-secondary/80 font-medium">Two Installments</span>
                 </label>
               </div>
             </div>
           )}
           <div className="pt-4 flex flex-col sm:flex-row items-center gap-3 border-t border-secondary/10 mt-8 w-full">
             ...
             >
               {event ? 'Save Changes' : <><Send className="w-4 h-4" /> Create & Send Link</>}
             </button>
           </div>
         </form>
      </div>
